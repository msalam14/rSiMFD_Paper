...
yq<-seq(1,2501,250)-1
yQ<-xq[-c(1,length(xq))]
xRowCol<-as.matrix(data.frame(expand.grid(xQ,yQ)))/2500
...
QEfun<-as.matrix(read.csv("QEfun.csv"))
QLfun<-as.matrix(read.csv("QLfun.csv"))
...
write.table(par_res,file=paste(dirP,"peso_dat_res9_",ifelse(c_opt,1,2),".txt",sep=""),col.names = FALSE,row.names = FALSE)
